...
describe("daily growth policy scoring", () => {
  it("rewards consistent median daily growth over slightly higher aggregate roi", () => {
    const consistent=scoreDailyGrowthPolicy({
      minRoiPct:4,avgRoiPct:6,totalTrades:200,avgHitRatePct:98,
    ...
    const lumpy=scoreDailyGrowthPolicy({
      minRoiPct:5,avgRoiPct:8,totalTrades:200,avgHitRatePct:95,
  ...
  it("penalizes losing-day concentration and drawdown", () => {
    const safe=scoreDailyGrowthPolicy({
      minRoiPct:5,avgRoiPct:7,totalTrades:150,avgHitRatePct:97,
  ...
  it("prefers the higher hit rate when daily risk and return are otherwise equal", () => {
    const higher=scoreDailyGrowthPolicy({
      minRoiPct:5,avgRoiPct:7,totalTrades:150,avgHitRatePct:99,
      profitableDayPct:95,medianDailyPnlPerDollar:0.06,avgDailyPnlPerDollar:0.07,
      worstDayPnlPerDollar:-0.05,maxDrawdownPerDollar:0.1,peakConcurrentTrades:2
    });
    const lower=scoreDailyGrowthPolicy({
      minRoiPct:5,avgRoiPct:7,totalTrades:150,avgHitRatePct:95,
      profitableDayPct:95,medianDailyPnlPerDollar:0.06,avgDailyPnlPerDollar:0.07,
      worstDayPnlPerDollar:-0.05,maxDrawdownPerDollar:0.1,peakConcurrentTrades:2
    });
