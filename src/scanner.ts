...
function shouldRetainForLogicalGraph(candidate: ScanCandidate) {
  if (isPoliticalCandidate(candidate)) return false;
  const q = candidate.question.toLowerCase();
  const hasSubject = /\b(bitcoin|btc|ethereum|ether|eth|solana|sol|xrp|ripple|dogecoin|doge|s&p\s*500|spx|nasdaq|ndx|dow|djia)\b/i.test(q);
  const hasDirection = /\b(above|over|higher than|greater than|at least|reach|hit|touch|exceed|below|under|lower than|less than|at most)\b/i.test(q);
  const hasNumber = /\$?\d[\d,.]*(?:\.\d+)?/.test(q);
  return hasSubject && hasDirection && hasNumber;
}

function compactLongDatedCandidate(candidate: ScanCandidate) {
  candidate.books = undefined;
  candidate.resolutionRules = null;
  candidate.marketSignals = undefined;
  candidate.externalEvidence = null;
  candidate.crossVenue = undefined;
  candidate.smartMoney = null;
  return candidate;
}
